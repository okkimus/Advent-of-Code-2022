...
lineReader.on('line', line => {
  if (readingStacks) {
    ...
    for (let i = 0; i < instruction.count; i++) {
      const highestItemPos = findHighestItemPosition(instruction.from)
      const itemToMovePosition = [highestItemPos[0] - instruction.count + 1 + i, instruction.from]
      const itemToMove = popItem(itemToMovePosition)
